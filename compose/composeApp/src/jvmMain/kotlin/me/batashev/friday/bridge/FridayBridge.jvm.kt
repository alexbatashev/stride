...
actual object FridayBridge {
    private val loaded: Boolean by lazy {
        runCatching {
            val explicit = System.getenv("FRIDAY_BRIDGE_LIB_DIR")
            if (!explicit.isNullOrBlank()) {
                val dir = File(explicit)
                System.load(File(dir, "libFridayBridge.so").absolutePath)
                System.load(File(dir, "libFridayBridgeJNI.so").absolutePath)
            } else {
