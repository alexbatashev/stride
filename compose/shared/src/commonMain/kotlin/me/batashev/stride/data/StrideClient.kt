...
sealed class StrideException : Exception() {
    data object NotConfigured : StrideException()
    data object Unauthorized : StrideException()
    data class Http(val code: Int) : StrideException()
    data object Transport : StrideException()
}

/**
 * The Stride cloud surface the threads feature needs, backed by Ktor. Reads the
 * active base URL and bearer token from [session] for every call.
 */
class StrideClient(private val session: Session) {
    ...
    private suspend fun authenticate(path: String, baseUrl: String, username: String, password: String) {
        ...
        } catch (e: StrideException) {
            throw e
        } catch (_: Throwable) {
            throw StrideException.Transport
        }
        validate(response)
    ...
    private suspend fun authed(
        ...
        val base = session.baseUrl ?: throw StrideException.NotConfigured
        val response = try {
    ...
    private fun validate(response: HttpResponse) {
        when (val code = response.status.value) {
            in 200..299 -> Unit
            401 -> throw StrideException.Unauthorized
            else -> throw StrideException.Http(code)
        }
    }
