...
struct FridayFilesService {
    let credentials: FridayCredentials
    private let session = URLSession(configuration: .default)

    init?(credentials: FridayCredentials? = FridayCredentials.current()) {
...
func providerTemporaryFile(named name: String, data: Data) throws -> URL {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("friday-fp", isDirectory: true)
