...
struct ChatFeature {
    ...
    enum Action: BindableAction {
        ...
        case reloadHistory
        case historyResponse(Result<[Message], FridayError>)
        case sendTapped
        case threadCreated(Result<SendResult, FridayError>)
        case sendFailed(FridayError)
        case cancelTapped
        ...
        Reduce { state, action in
            ...
            case .connect:
                ...
                return .run { send in
                    while !Task.isCancelled {
                        do {
                            for try await event in friday.events(threadID) {
            ...
            case .sendTapped:
                ...
                if let threadID = state.threadID {
                    return .run { send in
                        do {
                            _ = try await friday.sendMessage(threadID, content, [])
                    ...
                    return .run { send in
                        do {
                            let result = try await friday.createThread(content, projectID, [])
            ...
            case .cancelTapped:
                ...
                return .run { _ in try? await friday.cancelRun(threadID) }
            ...
            case let .approvalResponse(approved):
                ...
                return .run { send in
                    do {
                        try await friday.resolveApproval(threadID, approval.id, approved)
                    } catch {
    ...
    private func loadMessages(_ threadID: String) async -> Result<[Message], FridayError> {
        do {
            return .success(try await friday.listMessages(threadID))
        } catch let error as FridayError {
