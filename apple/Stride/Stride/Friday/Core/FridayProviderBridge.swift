...
final class FridayProviderBridge: @unchecked Sendable {
    static let shared = FridayProviderBridge()

    private init() {}

    #if canImport(FileProvider)
    static let domainIdentifier = NSFileProviderDomainIdentifier("me.batashev.Stride.friday-files")

    private var domain: NSFileProviderDomain {
        NSFileProviderDomain(identifier: Self.domainIdentifier, displayName: "Friday")
    }
    #endif
