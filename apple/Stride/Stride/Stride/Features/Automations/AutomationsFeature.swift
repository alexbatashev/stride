...
struct AutomationsFeature {
    ...
    enum Action: BindableAction {
        ...
        case refresh
        case load
        case loaded(Result<[Automation], StrideError>)
        case emailAccountsLoaded([EmailAccount])

        case automationSelected(Automation.ID?)
        case loadRuns(Automation.ID)
        case runsLoaded(id: Automation.ID, Result<[AutomationRun], StrideError>)
        ...
        case enabledUpdated(Result<Void, StrideError>)

        case deleteTapped(Automation)
        case confirmDelete
        case deleteFinished(Result<Void, StrideError>)

        case newTapped
        case create(PresentationAction<CreateAutomationFeature.Action>)
        case createFinished(Result<Automation, StrideError>)
    ...
    var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case .load:
                return .run { send in
                    await send(.loaded(await asyncResult { try await stride.listAutomations() }))
                }
            ...
            case let .loadRuns(id):
                return .run { send in
                    await send(.runsLoaded(id: id, await asyncResult { try await stride.listAutomationRuns(id) }))
                }
            ...
            case let .toggleEnabled(automation):
                ...
                return .run { send in
                    await send(.enabledUpdated(await asyncResult {
                        try await stride.setAutomationEnabled(id, enabled)
                    }))
                }
            ...
            case .confirmDelete:
                ...
                return .run { send in
                    await send(.deleteFinished(await asyncResult { try await stride.deleteAutomation(id) }))
                }
            ...
            case let .create(.presented(.delegate(.submit(draft)))):
                return .run { send in
                    await send(.createFinished(await asyncResult { try await stride.createAutomation(draft) }))
                }
    ...
    private func loadEmailAccounts() -> Effect<Action> {
        .run { send in
            let accounts = (try? await stride.listEmailAccounts()) ?? []
    ...
    private func pollRun(id: Automation.ID) -> Effect<Action> {
        .run { send in
            do {
                try await stride.runAutomation(id)
            } catch {
                await send(.setError(message(for: StrideError.from(error))))
                await send(.runFinished(id))
                return
            }
            for attempt in 0 ..< 16 {
                try? await clock.sleep(for: .milliseconds(attempt < 4 ? 600 : 1200))
                guard let runs = try? await stride.listAutomationRuns(id) else { continue }
    ...
    private func webhookURL(for id: Automation.ID) -> String {
        ...
        guard let base = stride.serverBaseURL() else { return path }
    ...
    private func createMessage(for error: StrideError) -> String {
