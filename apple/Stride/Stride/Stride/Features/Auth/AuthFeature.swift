...
struct AuthFeature {
    ...
    enum Action: BindableAction {
        ...
        case submitTapped
        case authResponse(Result<Void, StrideError>)
        ...
        Reduce { state, action in
            ...
            case .submitTapped:
                ...
                return .run { send in
                    do {
                        if mode == .login {
                            try await stride.login(url, username, password)
                        } else {
                            try await stride.register(url, username, password)
                        }
    ...
    private func message(for error: StrideError, mode: State.Mode) -> String {
        switch error {
        case .notConfigured:
...
/// Accepts bare hosts ("stride.example.com") and full URLs, defaulting to https.
