...
impl MountedVfs {
    ...
    /// Bridges a legacy [`WritableArea`] into the mount model. Transitional:
    /// stage 4 constructs the workspace and project grant directly.
    pub fn from_writable_area(vfs: Arc<Vfs>, owner: Uuid, area: WritableArea) -> Self {
        match area {
            WritableArea::Workspace(id) => Self::new(vfs, owner, Some(id), None),
            WritableArea::ProjectDir(prefix) => Self::new(vfs, owner, None, Some(prefix)),
        }
    }
...
/// A thread's writable area, used by higher layers until stage 4 rewires them
/// onto the mount model directly.
#[derive(Clone)]
pub enum WritableArea {
    /// A standalone workspace tree exposed at `/home/agent`.
    Workspace(Uuid),
