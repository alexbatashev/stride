...
function submitAnswer(host: HTMLElement, root: ShadowRoot): void {
  const custom = root.querySelector<HTMLInputElement>('input[type="text"]')?.value.trim() ?? "";
  const picked = root.querySelector<HTMLInputElement>('input[type="radio"]:checked');
  const answer = custom.length > 0 ? custom : (picked?.value ?? "");
  if (!answer) return;

  const customInput = root.querySelector<HTMLInputElement>('input[type="text"]');
  if (customInput) customInput.value = "";
  if (picked) picked.checked = false;

  host.dispatchEvent(
    new CustomEvent("quiz-response", { bubbles: true, composed: true, detail: { answer } }),
  );
}
