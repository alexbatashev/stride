...
export function AppToolCluster({ tools = [] }: { tools?: TimelineItem[] }): Component {
  ...
  return <><style>{styles}</style><div class="tools">{previousVisible && previous.map((tool) => <AppToolActivity key={tool.id} activityId={tool.id} seq={tool.seq} title={tool.toolName !== "" ? tool.toolName : "Tool output"} detail={tool.toolDetail} content={tool.text} status={tool.status} isError={tool.isError} subagentKey={tool.subagentKey ?? ""} />).join("")}{newest.map((tool) => <AppToolActivity key={tool.id} activityId={tool.id} seq={tool.seq} title={tool.toolName !== "" ? tool.toolName : "Tool output"} detail={tool.toolDetail} content={tool.text} status={tool.status} isError={tool.isError} subagentKey={tool.subagentKey ?? ""} />).join("")}{previous.length > 0 && <button class="toggle" type="button" aria-expanded={previousVisible ? "true" : "false"} onClick={() => { previousVisible = !previousVisible; }}><span class={`toggle-icon${previousVisible ? " expanded" : ""}`} aria-hidden="true"><IconChevronDown /></span><span>{previousVisible ? "Show fewer tool calls" : `+${previous.length} previous tool call${previous.length === 1 ? "" : "s"}`}</span></button>}</div></>;
}
