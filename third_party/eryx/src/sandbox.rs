...
impl Sandbox {
    ...
    async fn run_inner(
        ...
        // Run callbacks off the guest thread so blocking callback imports can
        // suspend that thread without starving the handler.
        ...
        let callback_handler = std::thread::spawn(move || {
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("eryx callback runtime");
            runtime.block_on(run_callback_handler(
                callback_rx,
                callbacks_arc,
                resource_limits,
                callback_secrets,
        ...
        let callback_invocations = callback_handler.join().unwrap_or(0);
